/**
 * Client-side parse of PLANNED_TERMS_JSON suffix (same rules as backend).
 */
...
export function extractPlannedTermsFromCoachReply(text) {
...
/**
 * Models often put several terms in one paragraph or one long sentence (few periods).
 * Break before common term introducers so each segment gets its own label.
 */
...
/**
 * Infer term buckets when the model omits PLANNED_TERMS_JSON (common with short replies).
 * Uses time phrases + per-sentence catalog codes.
 *
 * @param {string} text - Advisor reply (segment splitting uses this only).
 * @param {Array<{ id: string }>} courses
 * @param {string} [userHint] - Optional user message (e.g. “over 2 years”) for fallbacks only.
 * @returns {{ terms: { label: string, course_ids: string[] }[] } | null}
 */
export function inferAdvisedPlanFromProse(text, courses, userHint = '') {
  ...
  /** @type {{ label: string, course_ids: string[] }[]} */
  const terms = [];
  let rollingLabel = null;
  /** Which year recent prose refers to (1 or 2) for bare “spring/fall/semester” lines */
...
/**
 * If the API JSON lumps terms too coarsely (e.g. only “Fall” per year) but prose parses into more
 * terms with the same course set, use the finer partition.
 * Also prefer prose when API courses are a subset of inferred courses (bad/partial JSON ids).
 */
export function preferProsePartitionIfSameCourses(apiPlan, prosePlan) {
